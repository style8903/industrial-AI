...
PATTERN_SIZE = (9,6)
left_imgs = list(sorted(glob.glob('./stereo/case1/left*.png')))
right_imgs = list(sorted(glob.glob('./stereo/case1/right*.png')))
...
for left_imgs_path, right_imgs_path in zip(left_imgs, right_imgs):
    left_img = cv2.imread(left_imgs_path, cv2.IMREAD_GRAYSCALE)
    right_img = cv2.imread(right_imgs_path, cv2.IMREAD_GRAYSCALE)
    if img_size is None:
